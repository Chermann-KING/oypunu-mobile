...
export const useDictionary = () => {
  ...
  const categoryService = serviceProvider.categoryService;
  ...
  const loadMoreResults = useCallback(
    async (options: SearchOptions, currentResults: Word[]) => {
      try {
  ...
  const getWordSuggestions = useCallback(
    async (partial: string, language?: string) => {
      try {
  ...
  const loadRandomWords = useCallback(
    async (count?: number, language?: string) => {
      try {
  ...
  const loadRecentWords = useCallback(
    async (limit?: number, language?: string) => {
      try {
  ...
  const loadPopularWords = useCallback(
    async (limit?: number, language?: string) => {
      try {
  ...
  // Destructure store setters to keep stable function references
  const { setLoadingCategories, setCategories } = dataHook;
  ...
  const loadCategories = useCallback(
    ...
    [categoryService, setLoadingCategories, setCategories]
  );

  const { setLoadingLanguages, setLanguages } = dataHook;
  ...
  }, [dictionaryService, setLoadingLanguages, setLanguages]);
  ...
  const getApprovedWordCount = useCallback(
    async (language: string) => {
      try {
        return await dictionaryService.getApprovedWordCount(language);
      } catch (error) {
...
export const useDictionaryContributor = () => {
  ...
  const createWord = useCallback(
    async (data: CreateWordData) => {
      try {
        const word = await dictionaryService.createWord(data);
        return word;
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : "Erreur de création du mot";
  ...
  const updateWord = useCallback(
    async (id: string, data: Partial<CreateWordData>) => {
      try {
  ...
  const uploadWordAudio = useCallback(
    async (wordId: string, audioFile: File) => {
      try {
        const result = await dictionaryService.uploadWordAudio(
          wordId,
          audioFile
        );
        return result;
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Erreur d'upload audio";
  ...
  const createCategory = useCallback(
    async (data: Omit<Category, "id" | "wordCount">) => {
      try {
  ...
  const updateCategory = useCallback(
    async (id: string, data: Partial<Omit<Category, "id" | "wordCount">>) => {
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Erreur de mise à jour de catégorie";
    ...
    async (id: string) => {
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Erreur de suppression de catégorie";
  ...
  const proposeLanguage = useCallback(
    async (data: Omit<Language, "id" | "status">) => {
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Erreur de proposition de langue";
